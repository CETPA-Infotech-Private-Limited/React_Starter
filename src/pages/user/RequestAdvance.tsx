...
const useAdvanceColumns = (employees: any[], handleTopUpClick: (claim: any) => void) => {
  return useMemo(
    () => [
      {
        accessorKey: 'sno',
      ...
      //         onClick={() => handleTopUpClick(row.original)}
      //       >
      //         Top Up
      //       </Button>
      //     </div>
...
const RequestAdvance = () => {
  ...
  const advanceClaims = useMemo(() => (claimList || []).filter((claim) => claim.claimTypeName === 'Advance'), [claimList]);
  ...
  const handleTopUpClick = (claim: any) => {
    setSelectedClaim(claim);
    setActiveForm('topup');
    scrollToForm();
  };

  const columns = useAdvanceColumns(employees, handleTopUpClick);
