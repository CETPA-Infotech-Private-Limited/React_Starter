...
type Props = {
  pdfUrl?: string;
  onAmend?: () => void | Promise<void>;
  amendLoading?: boolean;
};

const DocumentPreview = ({ pdfUrl, onAmend , amendLoading}: Props) => {
  return (
    <Card className="relative shadow-lg border border-blue-200 rounded-xl overflow-hidden">
      ...
      <CardContent className="p-0 relative">
        ...
        <div className="p-4 flex justify-end gap-3 bg-gradient-to-t from-white via-white/95 to-transparent border-t border-blue-100">
          <Button
            className="bg-red-500 hover:bg-red-600 text-white font-semibold shadow-md rounded-lg px-6 py-2 disabled:opacity-60"
            onClick={onAmend}
            disabled={amendLoading}
          >
            {amendLoading ? "Submitting..." : "Tour Amendment"}
          </Button>

          <Button
            variant="outline"
            className="bg-white border-2 border-blue-500 text-blue-700 hover:bg-blue-50 font-semibold shadow-md rounded-lg px-6 py-2"
          >
