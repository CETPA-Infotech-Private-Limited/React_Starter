      ...
      <thead className="bg-gray-50">
        <tr>
          {headers.map((header, index) => (
            <th
              key={index}
              className="px-4 py-3 text-left text-xs font-medium text-primary drop-shadow uppercase tracking-wider border-b border-gray-300"
            >
              {header}
            </th>
          ))}
...
export const StatusBadge = ({ status, type = 'default' }) => {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${variants[type]}`}>
      {status}
    </span>
  );
};
...
export const BillItemDisplayRow = ({
  ...
  onApprovalChange: (val: string) => void;
}) => (
  ...
  return (
    <>
      <tr className="hover:bg-gray-50">
        ...
        <td className="px-4 py-3 text-sm text-gray-900 border-r border-gray-200 text-right">
